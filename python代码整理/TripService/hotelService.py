...
def view_hotel(viewpoint_name):
    ...
    for hotel_data in range(shape(data)[0]):
        ...
        if len(data[hotel_data]['biz_ext']['cost']):
            hotel_minprice =(str(data[hotel_data]['biz_ext']['cost'])).encode('utf-8')
        else:
            hotel_minprice = '无'
...
def hotel_price_recom(data,viewpoint_location):
    ...
    priceList = []
    for hotel_data in range(shape(data)[0]):
        if len(''.join(data[hotel_data]['biz_ext']['cost'])):
            hotelPrice = float((data[hotel_data]['biz_ext']['cost']).encode('utf-8')) + 1
        else:
            hotelPrice = 100000
