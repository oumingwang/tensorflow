...
#选择景点按钮
def selViewpoint(viewDict):
    str1 = str(var.get())
    i = int(str1)
    name = (viewDict[i][0]).encode('utf-8')
    msg.title(name + '景点介绍')
    viewpoint_name = viewDict[i][0]
    viewpoint_grade = viewDict[i][1]
    viewpoint_price_min = viewDict[i][2]
    viewpoint_address = viewDict[i][3]
    #viewpoint_imgurl = tripName['result'][1]['imgurl']
    viewpoint_location = viewDict[i][4]
    '''
...
def view_hotel(viewpoint_name,viewpoint_location):
    ...
    hotelDict = hotelLocation.hotelRequest(hotel_name,viewpoint_location)
    ...
    for hotel_data in range(shape(data)[0]):
        ...
        if len(data[hotel_data]['biz_ext']['lowest_price']):
            ...
            #hotelMinPrice = 30000
        '''
    for i in range(len(hotelDict)):
        Radiobutton(root,text='最短距离推荐',command=lambda : hotel_distance_recom(hotelDict,viewpoint_location)).grid(row = 3,column=0)
        Radiobutton(root, text='最低消费推荐',command=lambda : hotel_price_recom(hotelDict,viewpoint_location)).grid(row=4, column=0)
        Radiobutton(root, text='最佳评分推荐',command=lambda : hotel_rate_recom(hotelDict,viewpoint_location)).grid(row=5, column=0)
        Radiobutton(root, text='性价比推荐', command=lambda: hotel_best_recom(hotelDict,viewpoint_location)).grid(row=6, column=0)
        Radiobutton(root, text=["宾馆名称：" + hotel_name,'宾馆经纬度：' + hotelDict[i][1],hotelDict[i][2],"宾馆评分：" + hotelDict[i][4].encode("utf-8"),"宾馆最低消费：" + hotelDict[i][5],'距离：'+ str(hotelDict[i][2])], variable=var, value=i,command=lambda : selHotel(data,viewpoint_location)).grid(row=i+2,column=1)
...
def hotel_best_recom(data,viewpoint_location):
    ...
    tkMessageBox.showinfo("性价比酒店推荐：", [data[bestNumIndex]['name'].encode('utf-8'),
                                           ...
                                           '宾馆电话：' + data[bestNumIndex]['tel'].encode('utf-8'),
...
def submit():
    #trip = Tk()
    #trip.title('Hello')
    viewDict = Trip.tripRecom(cityValue.get().encode('utf-8'))
    for trip_name in range(len(viewDict)):
        name = viewDict[trip_name][0]
        Radiobutton(root, text=[name], variable=var, value=trip_name, command=lambda :selViewpoint(viewDict)).grid(row=2+trip_name,column=1)
    #trip.mainloop()
